...
// Discussion Page Component - moved outside to prevent re-creation
...
// Auth Modal Component - moved outside
...
// Preferences Modal Component - moved outside
...
const RinON = () => {
    ...
    const loadPosts = async (topicId) => {
        try {
            const { data } = await supabase.from('posts').select('*').eq('topic_id', topicId).order('created_at', { ascending: false });
            setTopicPosts(data || []);
        } catch (err) {
    ...
    const handleSignup = async (email, password, displayName) => {
        ...
        if (!error && data.user) {
            await supabase.from('user_profiles').insert([{
                id: data.user.id, display_name: displayName, preferences: []
            }]);
    ...
    const deleteTopic = async (id) => {
        if (window.confirm(t('Jeni i sigurt që dëshironi të fshini këtë temë?', 'Are you sure you want to delete this topic?'))) {
            ...
            if (!error) loadTopics();
            else alert(t('Gabim në fshirje', 'Error deleting'));
        }
    };
    ...
    const handleSubmitArticle = async () => {
        ...
        const article = {
            title_al: formData.titleAl,
            title_en: formData.titleEn || formData.titleAl,
            content_al: formData.contentAl,
            content_en: formData.contentEn || formData.contentAl,
        ...
        if (!error && data) {
            ...
            alert(t('Gabim: ' + error.message, 'Error: ' + error.message));
        }
    };
    ...
    const handleSubmitEvent = async () => {
        ...
        const event = {
            title_al: eventFormData.titleAl,
            title_en: eventFormData.titleEn || eventFormData.titleAl,
            date_al: eventFormData.dateAl,
            date_en: eventFormData.dateEn || eventFormData.dateAl,
            type: eventFormData.type,
            desc_al: eventFormData.descAl,
            desc_en: eventFormData.descEn || eventFormData.descAl,
    ...
    const handleSubmitTopic = async () => {
        ...
        const topic = {
            title_al: topicFormData.titleAl,
            title_en: topicFormData.titleEn || topicFormData.titleAl,
            description_al: topicFormData.descriptionAl,
            description_en: topicFormData.descriptionEn || topicFormData.descriptionAl,
            created_by: user?.id
        };
    ...
    const handleSubmitPartner = async () => {
        ...
        const partner = {
            name_al: partnerFormData.nameAl,
            name_en: partnerFormData.nameEn,
            description_al: partnerFormData.descriptionAl,
            description_en: partnerFormData.descriptionEn,
            vision_al: partnerFormData.visionAl,
            vision_en: partnerFormData.visionEn,
            goals_al: partnerFormData.goalsAl,
            goals_en: partnerFormData.goalsEn,
    ...
    const PartnersPage = () => (
        <div className="max-w-7xl mx-auto px-4 py-12">
            <div className="text-center mb-12">
                <h1 className={`text-5xl font-bold mb-4 ${darkMode ? 'bg-gradient-to-r from-purple-400 via-pink-400 to-orange-400 bg-clip-text text-transparent' : 'text-gray-900'}`}>{t('Bashkëpunime', 'Cooperations')}</h1>
                <p className={`text-xl ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{t('Organizatat partnere të RinON', 'RinON Partner Organizations')}</p>
            </div>
            ...
            {partners.length === 0 ? (
                <div className="text-center py-20">
                    ...
                    <h3 className={`text-2xl font-bold mb-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                        {t('Asnjë partner ende', 'No partners yet')}
                    </h3>
                    <p className={darkMode ? 'text-gray-400' : 'text-gray-600'}>
                        {t('Partnerët do të shfaqen këtu kur të shtohen', 'Partners will appear here when added')}
                    </p>
                </div>
            ) : (
